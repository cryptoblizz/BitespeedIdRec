...
public interface ContactRepository extends JpaRepository<Contact, Integer> {
    ...
    @Modifying
    @Transactional
    @Query("UPDATE Contact c SET c.linkPrecedence = :linkPrecedence, c.linkedId = :excludedContactId WHERE c <> :excludedContact AND (c.linkedId = :rId1 OR c.id = :rId1)")
    void updateRecordsTwoPrimary(@Param("linkPrecedence") String linkPrecedence, @Param("excludedContactId") Integer excludedContactId, @Param("rId1") Integer rId1, @Param("excludedContact") Contact excludedContact);

    @Query("SELECT DISTINCT c FROM Contact c WHERE c.linkedId = :linkedId or c.id = :linkedId order by c.id")
    List<Contact> getEmailsByLinkedId(Integer linkedId);

    @Query("SELECT DISTINCT c FROM Contact c WHERE c.linkedId = :linkedId or c.id = :linkedId order by c.id")
    List<Contact> getPhoneNumbersByLinkedId(Integer linkedId);
    ...
    @Query("(SELECT c FROM Contact c WHERE c.linkedId = :linkedId order by c.id)")
    List<Contact> getSecondaryIDs(Integer linkedId);
}
